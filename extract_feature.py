...
def launch(config: DictConfig) -> None:
    ...
    config.experiment.name = "extract_feature"
    config.experiment.builder._target_ = "src.experiment.extract_feature.Experiment"
    config.dataloader.train_config.dataloader.batch_size = 1024
    return run(config)
